...
<form id="salaryForm" class="form-design" action="{{url('grade_salary_master')}}" method="POST">
    ...
    <div class="modal-body">
        <div class="row mb-3 mt-4">
            ...
            <div class="col-sm-9">
                <select class="form-select" name="grade" id="grade_sal">
            ...
            <section class="main-section">
                <section class="cal2">
                    <div class="design-button">
                        <h4>Grade Salary Head</h4>
                        <div class="cal-button " id="append_grade_data">
                ...
                <section class="cal2">
                    <div class="design-button">
                        <h4>Master Salary Head</h4>
                        <div class="cal-button ">
                            @foreach($all_master_head as $val)
                            <?php $head_title = '{M-' . $val->head_title . '}' ?>
                            <a onclick="showFormula('{{$head_title}}')" data-salary-formula="" class="button-deisgn">M-{{ $val->head_title }}</a>
                            @endforeach
                        </div>
                    </div>
                </section>
...
<script>
    $('#grade_sal').on('change', function() {
        var grade_id = this.value;
        $.ajax({
            url: "{{ url('get_grade_data') }}",
            type: "POST",
            data: {
                _token: '{{ csrf_token() }}',
                grade: grade_id
